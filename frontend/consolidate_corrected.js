...
const outputDir = path.join(baseDir, 'CONSOLIDADO_FINAL');

console.log('📊 Consolidando TODO en una sola hoja (CORREGIDO)...\n');
...
for (let r = 0; r <= acuRange.e.r; r++) {
  ...
  const cellR = acuSheet[XLSX.utils.encode_cell({ r, c: 17 })]; // PARCIAL (del ACU)
  ...
  const valQ = cellQ ? (cellQ.v || '') : ''; // PRECIO
  const valR = cellR ? (cellR.v || '') : ''; // PARCIAL
  ...
  if (currentPartida && currentClasificador && valA && /^\d+$/.test(valA) && valJ) {
    ...
    const precio = parseFloat(valQ) || 0; // PRECIO UNITARIO
    const parcial = parseFloat(valR) || 0; // PARCIAL DEL ACU
...
console.log(`   • Precio: Unitario (columna Q del ACU)`);
console.log(`   • Parcial: Directo del ACU (columna R)`);
console.log(`   • Rendimiento: 40% con valores, 60% vacío (según ACU)`);
