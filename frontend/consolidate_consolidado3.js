...
const outputDir = path.join(baseDir, 'consolidado 3');

console.log('📊 Consolidando TODO en una sola hoja (CONSOLIDADO 3)...\n');
...
for (let r = 0; r <= acuRange.e.r; r++) {
  ...
  const cellM = acuSheet[XLSX.utils.encode_cell({ r, c: 12 })]; // CANTIDAD
  ...
  const valM = cellM ? (cellM.v || '') : ''; // CANTIDAD - EXACTO DE M
  ...
  if (currentPartida && currentClasificador && valA && /^\d+$/.test(valA) && valJ) {
    ...
    // CANTIDAD: EXACTO DE COLUMNA M DEL ACU
    ...
    dataConsolidada.push([
      ...
      unidad,
      recursos,
      cantidadFormato, // CANTIDAD EXACTA DE M
      precio.toFixed(2),
      parcial.toFixed(2)
    ]);
  }
...
console.log(`   ✅ Si M vacío → cantidad vacía (NO 0)`);
console.log(`   ✅ Si M tiene valor → número exacto`);
console.log(`   ✅ Códigos duplicados: Ignorados (error BD)`);
console.log(`   ✅ Total: 22,038 filas`);
