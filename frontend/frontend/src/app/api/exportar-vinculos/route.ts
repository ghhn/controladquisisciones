...
export async function GET() {
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    // HOJA 6: AJUSTE MANUAL - COMPARATIVO EXPEDIENTE vs MODIFICADO
    // ═══════════════════════════════════════════════════════════════
    const ajusteManualQuery = await client.query(`
      SELECT
        a.id,
        a.codigo_insumo,
        a.item_partida,
        ...
        p.cantidad_p as metrado,
        a.cantidad_p as cantidad_expediente,
        a.parcial_p as incidencia_expediente,
        COALESCE(a.cantidad_c, a.cantidad_p) as cantidad_modificada,
        COALESCE(a.parcial_c, a.parcial_p) as incidencia_modificada,
        COALESCE(a.parcial_c, a.parcial_p) - a.parcial_p as variacion_incidencia,
        COALESCE(a.cantidad_c, a.cantidad_p) - a.cantidad_p as variacion_cantidad
      FROM acus a
      LEFT JOIN partidas_p p ON a.item_partida = p.item
      ORDER BY a.item_partida, a.codigo_insumo
    `);

    const ws6 = workbook.addWorksheet('6. Ajuste Manual');
    ws6.columns = [
      { header: 'Código Item', key: 'codigo_insumo', width: 14 },
      { header: 'ITEM', key: 'item_partida', width: 12 },
      { header: 'PARTIDA', key: 'partida_desc', width: 35 },
      { header: 'METRADO', key: 'metrado', width: 12 },
      { header: 'Descripción Insumo', key: 'descripcion_insumo', width: 35 },
      { header: 'INCIDENCIA SEGÚN EXPEDIENTE', key: 'incidencia_expediente', width: 16 },
      { header: 'CANTIDAD SEGÚN EXPEDIENTE', key: 'cantidad_expediente', width: 16 },
      { header: 'INCIDENCIA MODIFICADO', key: 'incidencia_modificada', width: 16 },
      { header: 'CANTIDAD MODIFICADA', key: 'cantidad_modificada', width: 16 },
      { header: 'VARIACIÓN DE INCIDENCIA', key: 'variacion_incidencia', width: 16 },
      { header: 'VARIACIÓN CANTIDAD', key: 'variacion_cantidad', width: 16 }
    ];

    const headerRow6 = ws6.getRow(1);
    headerRow6.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF203864' } };
    headerRow6.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 10 };
    headerRow6.height = 25;
    headerRow6.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };

    let rowNum6 = 2;
    let currentPartida6 = '';
    let useAlternado6 = false;

    ajusteManualQuery.rows.forEach((row: any) => {
      if (row.item_partida !== currentPartida6) {
        currentPartida6 = row.item_partida;
        useAlternado6 = !useAlternado6;
      }

      const wsRow = ws6.getRow(rowNum6);
      wsRow.values = {
        codigo_insumo: row.codigo_insumo || '',
        item_partida: row.item_partida || '',
        partida_desc: row.partida_desc || '',
        metrado: row.metrado || '',
        descripcion_insumo: row.descripcion_insumo || '',
        incidencia_expediente: row.incidencia_expediente || 0,
        cantidad_expediente: row.cantidad_expediente || 0,
        incidencia_modificada: row.incidencia_modificada || 0,
        cantidad_modificada: row.cantidad_modificada || 0,
        variacion_incidencia: row.variacion_incidencia || 0,
        variacion_cantidad: row.variacion_cantidad || 0
      };

      const bgColor = useAlternado6 ? 'FFF5F5F5' : 'FFFFFFFF';
      Array.from({ length: 11 }).forEach((_, colIdx) => {
        const cell = wsRow.getCell(colIdx + 1);
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: bgColor } };
        cell.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
        addBorder(cell);
        ...
        // Formato para números
        if ([4, 6, 7, 8, 9, 10, 11].includes(colIdx + 1)) {
          cell.alignment = { horizontal: 'right', vertical: 'middle' };
          cell.numFmt = '#,##0.0000';
        }

        // Resaltar variaciones
        if ([10, 11].includes(colIdx + 1) && row.variacion_incidencia !== 0 || row.variacion_cantidad !== 0) {
          cell.font = { bold: true };
        }
      });
